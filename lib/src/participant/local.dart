...
class LocalParticipant extends Participant<LocalTrackPublication> {
  ...
  void _broadcastStateChanged() {
    ...
    // Wrap in try-catch to prevent unhandled exceptions from crashing the app.
    unawaited(
      setScreenShareEnabled(isEnabled).catchError((error) {
        logger.warning('Failed to set screen share enabled: $error');
      }),
    );
  }
