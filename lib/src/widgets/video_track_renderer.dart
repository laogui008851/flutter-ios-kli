...
class _VideoTrackRendererState extends State<VideoTrackRenderer> {
  ...
  // Tracks whether setFocusPoint/setExposurePoint are supported on this device.
  // Once a platform error is encountered, further calls are skipped to avoid
  // spamming error logs.
  ...
  void onViewFinderTap(TapDownDetails details, BoxConstraints constraints) {
    if (!_focusPointSupported && !_exposurePointSupported) return;
    ...
    // Don't block the UI. Use async closures with try-catch to gracefully
    // handle platform errors (e.g. null DeviceOrientation on some Android
    // devices in flutter_webrtc's CameraUtils). After the first failure,
    // disable further attempts to avoid spamming error logs.
    if (_focusPointSupported) {
      unawaited(() async {
        try {
          await rtc.Helper.setFocusPoint(videoTrack, point);
        } catch (e) {
          logger.warning('setFocusPoint failed, disabling: $e');
    ...
    if (_exposurePointSupported) {
      unawaited(() async {
        try {
          await rtc.Helper.setExposurePoint(videoTrack, point);
        } catch (e) {
          logger.warning('setExposurePoint failed, disabling: $e');
